...
export class Core_UXFormControl extends Core_UXElement {
    ...
    /**
     * insertBefore anchor must be a direct child of `.core-field`.
     * @param {HTMLElement} field
     * @param {ChildNode|null} preferred
     * @returns {ChildNode|null}
     */
    _resolveFieldInsertAnchor(field, preferred) {
        if (!preferred) {
            return field.firstChild;
        }
        if (preferred.parentNode === field) {
            return preferred;
        }
        let node = preferred;
        while (node && node.parentNode !== field && field.contains(node)) {
    ...
    _getLabelInsertBefore() {
        ...
        const preferred = wrap || control;
        return this._resolveFieldInsertAnchor(field, preferred);
    }
